...
BOOL password_valid(wchar_t * pwd, int length)
{
	...
	// Open log
	HANDLE h = pshk_log_open();

	if (length > 0)
	{
		size_t converted = 0;
		wchar_t * tmp = (wchar_t*)malloc(length * sizeof(wchar_t));
		//wcstombs_s(&converted, tmp, length, pwd, _TRUNCATE);
		for (int i = 0; i < length; i++)
		{
			tmp[i] = towlower(pwd[i]);
		}

		pshk_log_write_w(h, tmp);

		const wchar_t * word1 = L"li";
		const wchar_t * word2 = L"fung";

		if (has_child(tmp, length, word1, 2) != 0)
		{
			return -1;
		}
		if (has_child(tmp, length, word2, 2) != 0)
		{
			return -1;
		}
		return 0;
